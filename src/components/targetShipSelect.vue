...
const emit = defineEmits(["targetShipCordinates"]);
const columnSelected = ref("");
const rowSelected = ref(0);

const targetShip = () => {
  emit("targetShipCordinates", {
...
<template>
  <div class="flex flex-col gap-4 text-left">
    <div>
      <h2 class="text-2xl font-bold">Select a Ship to Target</h2>
      <p>Choose a column and row to target a ship</p>
    </div>

    <form class="flex flex-row gap-4" @submit.prevent="targetShip">
      ...
      <button :disabled="columnSelected === '' || rowSelected === 0">
        Target Ship
      </button>
    </form>
  </div>
